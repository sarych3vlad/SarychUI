-- Cromulent Wrapper for SarychUI
-- Manages loading and enabling/disabling of Cromulent addon

local ADDON_NAME = "Cromulent"
local addonPath = "Interface\\AddOns\\SarychUI\\addons\\Cromulent\\"
...
CromulentEnabled = CromulentEnabled or true;
...
function wrapper:Initialize()
	...
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons then
		if SarychUI.db.profile.addons[ADDON_NAME] and SarychUI.db.profile.addons[ADDON_NAME].enabled then
			...
			CromulentEnabled = true
			self.enabled = true
			self.loaded = true
		else
			-- Set global enabled state
			CromulentEnabled = false
			self.enabled = false
		end
	else
...
function wrapper:Enable()
	...
	-- Enable the addon if it exists (use Enable() which checks state internally)
	if Cromulent and Cromulent.Enable then
		Cromulent:Enable()
	end
end
...
function wrapper:Disable()
	...
	-- Disable the addon if it exists (use Disable() which checks state internally)
	if Cromulent and Cromulent.Disable then
		Cromulent:Disable()
	end
end
	...
	return {
		...
		args = {
			enabled = {
				type = "toggle",
				name = "Включить",
				desc = "Включить/выключить Cromulent",
				order = 1,
				get = function()
			...
			description = {
				type = "description",
				name = "Показывает информацию о зоне на карте мира (ZoneInfo on the World Map).\n\nАвтор: " .. (self.author or "Неизвестен") .. "\nВерсия: " .. (self.version or "Неизвестна"),
				order = 2,
				width = "full",
			},
		},

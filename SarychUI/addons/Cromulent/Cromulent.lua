...
local function IsEnabled()
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.Cromulent then
		return SarychUI.db.profile.addons.Cromulent.enabled ~= false;
	end
...
function Cromulent:OnEnable()
	...
	self.frame:Show()
	-- Check if already hooked before hooking
	if not self:IsHooked(WorldMapButton, "OnUpdate") then
		self:SecureHookScript(WorldMapButton, "OnUpdate", "WorldMapButton_OnUpdate")
	end
end

function Cromulent:OnDisable()
	-- Unhook the script if it's hooked
	if self:IsHooked(WorldMapButton, "OnUpdate") then

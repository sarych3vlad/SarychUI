...
local EventFuncs = {
  VARIABLES_LOADED = function()
    local version = GetAddOnMetadata("BaudBag","Version") or (SarychUI_BaudBag and SarychUI_BaudBag.version) or "1.4.9";
    BaudBagVersionText:SetText("Version "..tostring(version));
...
function BaudBagUpdateContainer(Container)
  ...
  for _, SubBag in ipairs(Container.Bags)do
    if(Container.BagSet~=2)or BankOpen then
      ...
      local cacheBag = BaudBag_Cache[SubBag:GetID()];
      Size = cacheBag and cacheBag.Size or 0;
    end
    SubBag.size = Size;
...
function BaudBagOptionsFrame_OnLoad(self)
  ...
  SlashCmdList[Prefix.."_Options"] = function()
    if SarychUI_BaudBag and SarychUI_BaudBag.ShowOptions then
      SarychUI_BaudBag:ShowOptions();
    else
      BaudBagOptionsFrame:Show();
    end
  end

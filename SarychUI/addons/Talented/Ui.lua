...
do
	...
	local function Layout(frame, width, height)
		local headerH = Talented.TREE_HEADER_HEIGHT or 0
		-- No Blizzard scrollbar gutter: scale 320-wide art to full column width
		local body_height = height - headerH
		if body_height < 1 then body_height = height end
		local texture_height = body_height / (256 + 75)
		local texture_width = width / 320
		...
		frame.topleft:SetPoint("TOPLEFT", frame, "TOPLEFT", 0, -headerH)
...
do
	...
	-- Slightly smaller than Blizzard 37/32 so columns breathe; links scale with icons.
	local TALENT_ICON_SIZE = 30
	local TALENT_RANK_SIZE = 26

	local function MakeRankFrame(button, anchor)
		local t = CreateTexture(button, "OVERLAY", "Interface\\AddOns\\SarychUI\\addons\\Talented\\Textures\\border")
		t:SetSize(TALENT_RANK_SIZE, TALENT_RANK_SIZE)
	...
	local function NewButton(parent)
		...
		button:SetSize(TALENT_ICON_SIZE, TALENT_ICON_SIZE)
		...
		t:SetSize(TALENT_ICON_SIZE - 1, TALENT_ICON_SIZE - 1)
		t:SetPoint("CENTER")
...
do
	...
	local function NewBranch(parent)
		...
		t:SetTexture("Interface\\AddOns\\SarychUI\\addons\\Talented\\Textures\\branches-normal")
		t:SetSize(LINK_SIZE, LINK_SIZE)
	...
	function Talented:MakeBranch(parent)
		...
		branch:SetSize(LINK_SIZE, LINK_SIZE)
		return branch
	end
end
...
do
	...
	local function NewArrow(parent)
		...
		t:SetTexture("Interface\\AddOns\\SarychUI\\addons\\Talented\\Textures\\arrows-normal")
		t:SetSize(LINK_SIZE, LINK_SIZE)
		t:SetVertexColor(NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b)
	...
	function Talented:MakeArrow(parent)
		...
		arrow:SetSize(LINK_SIZE, LINK_SIZE)
		return arrow
	end
end
...
do
	...
	-- Link textures are 26px (was 32); scale placement offsets to match.
	local function LS(n)
		return math.floor(n * (26 / 32) + 0.5)
	end
	...
	local function DrawVerticalLine(list, parent, offset, base_row, base_column, row, column)
		...
		for i = row + 1, base_row - 1 do
			...
			branch:SetPoint("TOPLEFT", x + LS(2), y + LS(32))
			list[#list + 1] = branch
			SetTextureCoords(branch, "branch", "top")
			branch = Talented:MakeBranch(parent)
			branch:SetPoint("TOPLEFT", x + LS(2), y)
			list[#list + 1] = branch
			SetTextureCoords(branch, "branch", "top")
		end
		...
		arrow:SetPoint("TOPLEFT", x + LS(2), y + LS(16))
		list[#list + 1] = arrow
	...
	local function DrawHorizontalLine(list, parent, offset, base_row, base_column, row, column)
		...
		for i = min(base_column, column) + 1, max(base_column, column) - 1 do
			...
			local branch = Talented:MakeBranch(parent)
			branch:SetPoint("TOPLEFT", x - LS(32), y - LS(2))
			list[#list + 1] = branch
			SetTextureCoords(branch, "branch", "left")
			branch = Talented:MakeBranch(parent)
			branch:SetPoint("TOPLEFT", x, y - LS(2))
			list[#list + 1] = branch
			SetTextureCoords(branch, "branch", "left")
		end
		local x, y = offset(base_row, base_column)
		...
		if base_column < column then
			...
			arrow:SetPoint("TOPLEFT", x + LS(20), y - LS(2))
			branch:SetPoint("TOPLEFT", x + LS(32), y - LS(2))
		else
			SetTextureCoords(arrow, "arrow", "left")
			arrow:SetPoint("TOPLEFT", x - LS(15), y - LS(2))
			branch:SetPoint("TOPLEFT", x - LS(32), y - LS(2))
		end
		list[#list + 1] = arrow
		return true
	...
	local function DrawHorizontalVerticalLine(list, parent, offset, base_row, base_column, row, column)
		...
		local branch = Talented:MakeBranch(parent)
		branch:SetPoint("TOPLEFT", x + LS(2), y - LS(2))
		list[#list + 1] = branch
		local branch2 = Talented:MakeBranch(parent)
		SetTextureCoords(branch2, "branch", "left")
		list[#list + 1] = branch2
		if base_column < column then
			branch2:SetPoint("TOPLEFT", x + LS(35), y - LS(2))
			SetTextureCoords(branch, "branch", "topleft")
		else
			branch2:SetPoint("TOPLEFT", x - LS(29), y - LS(2))
		...
		for i = row + 1, base_row - 1 do
			...
			b:SetPoint("TOPLEFT", xofs + LS(2), yofs + LS(32))
			list[#list + 1] = b
			SetTextureCoords(b, "branch", "top")
			b = Talented:MakeBranch(parent)
			b:SetPoint("TOPLEFT", xofs + LS(2), yofs)
			list[#list + 1] = b
			SetTextureCoords(b, "branch", "top")
		end
...
do
	...
	function Talented:MakeTemplateMenu()
		...
		if entry then
			entry.disabled = (not self.pet_current) or (Talented.PetTalentsAvailable and not Talented:PetTalentsAvailable())
	...
	function Talented:MakeActionMenu()
		...
		do
			local apply = self:GetNamedMenu("Apply")
			if templateTalentGroup or (restricted and pet_restricted) then
				apply.disabled = true
			elseif self.CanApplyTemplate then
				apply.disabled = not self:CanApplyTemplate(self.template)
			else
				apply.disabled = restricted and pet_restricted
			end
		end
	...
	function Talented:GetDropdownFrame(frame)
		...
		dropdown.xOffset = 0
		dropdown.yOffset = 4 -- overlap button so leave-timer does not fire in the gap
...
do
	...
	local function TabFrame_OnClick(self, button)
		...
		if button == "RightButton" then
			...
			local template
			if info.pet then
				if not (Talented.PetTalentsAvailable and Talented:PetTalentsAvailable()) then
					Talented.pet_current = nil
					self:Hide()
					return
				end
				if Talented.PET_TALENT_UPDATE then
					Talented:PET_TALENT_UPDATE()
				end
	...
	local function Tabs_Update(self)
		...
		if GetNumTalentGroups() > 1 then
			...
			spec2:Show()
			spec2:SetPoint("TOP", anchor, "BOTTOM", 0, -4)
			spec2:Update()
			anchor = spec2
		else
			self.spec2:Hide()
		end
		local pet = Talented.pet_current and Talented.PetTalentsAvailable and Talented:PetTalentsAvailable()
		if pet then
			local petspec1 = self.petspec1
			petspec1:Show()
			petspec1:Update()
			petspec1:SetPoint("TOP", anchor, "BOTTOM", 0, -4)
		else
			self.petspec1:Hide()
		end
	end
	...
	local function Tabs_OnEvent(self, event, ...)
		if event ~= "UNIT_PET" or (...) == "player" then
			if (event == "UNIT_PET" or event == "PET_TALENT_UPDATE") and Talented.PET_TALENT_UPDATE then
				Talented:PET_TALENT_UPDATE()
			end
			Tabs_Update(self)
		end

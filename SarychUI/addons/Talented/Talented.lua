local Talented = LibStub("AceAddon-3.0"):NewAddon("Talented", "AceEvent-3.0", "AceHook-3.0", "AceConsole-3.0")
...
do
	-- Quiet: no chat spam from the addon
	function Talented:Print(...)
	end
	...
	function Talented:OnInitialize()
		...
		-- Embedded in SarychUI: defer Ace enable until wrapper reads db.
		if SarychUI then
	...
	function Talented:RegisterInterfaceOptions()
		if self._suiBlizRegistered then
			return
		end
		self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Talented", "Talented")
		self._suiBlizRegistered = true
	end
	...
	function Talented:OpenTemplate(template)
		...
		if not base:IsVisible() then
			-- Don't override with the active spec in OnShow
			self.talentedSkipActiveOnShow = true
			ShowUIPanel(base)
		end
	end
	...
	-- Open on the currently active talent group (primary/secondary)
	function Talented:OpenActiveSpec()
		self:CreateBaseFrame()
		self:UpdatePlayerSpecs()
		local active = self:GetActiveSpec()
		if active then
			self:SetTemplate(active)
		else
			self:SetTemplate()
		end
		self:UpdateView()
		-- Refresh spec tab icons / checked state after template switch
		if self.tabs then
			if self.tabs.Update then self.tabs:Update() end
			if self.tabs.UpdateCheck then
				self.tabs:UpdateCheck(self.template or active)
			end
			self.tabs:Show()
		end
	end
	...
	function Talented:OnEnable()
		local enabled = true
		if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.Talented then
			enabled = SarychUI.db.profile.addons.Talented.enabled ~= false
		elseif TalentedEnabled ~= nil then
			enabled = TalentedEnabled ~= false
		end
		if not enabled then
			return
		end
...
do
	...
	function Talented:CheckSpellData(class)
		...
		for tab = 1, GetNumTalentTabs() do
			...
			local tabname, icon, _, background = GetTalentTabInfo(tab)
			tabdata[tab].name = tabname -- no need to mark invalid for these
			tabdata[tab].icon = icon
...
do
	...
	function Talented:GetLearnedTalentRank(template, tab, index)
		if not template then return 0 end
		local pet = not RAID_CLASS_COLORS[template.class]
		if template.talentGroup then
			return select(5, GetTalentInfo(tab, index, nil, pet, template.talentGroup)) or 0
		end
		local current = pet and self.pet_current or self:GetActiveSpec()
		return (current and current[tab] and current[tab][index]) or 0
	end

	function Talented:IsTalentPreviewDirty(template)
		if not template or not template.talentGroup then return false end
		local class = template.class
		if not class then return false end
		for tab, tree in ipairs(self:UncompressSpellData(class)) do
			local ttab = template[tab]
			if ttab then
				for index = 1, #tree do
					if (ttab[index] or 0) ~= self:GetLearnedTalentRank(template, tab, index) then
						return true
					end
				end
			end
	...
	function Talented:GetLearnedPointCount(template)
		if not template then return 0 end
		local pet = not RAID_CLASS_COLORS[template.class]
		local group = template.talentGroup or GetActiveTalentGroup(nil, pet)
		local total = 0
		for tab = 1, GetNumTalentTabs(nil, pet) do
			total = total + (select(3, GetTalentTabInfo(tab, nil, pet, group)) or 0)
		end
		return total
	end

	function Talented:GetAvailableTalentPointsForTemplate(template)
		if not template then return 0 end
		local pet = not RAID_CLASS_COLORS[template.class]
		local group = template.talentGroup or GetActiveTalentGroup(nil, pet)
		local apiFree = GetUnspentTalentPoints(nil, pet, group) or 0
		...
		local maxPoints = level >= 10 and math.min(level - 9, self.max_talent_points or 71) or 0
		local expectedFree = maxPoints - self:GetLearnedPointCount(template)
		if expectedFree < 0 then expectedFree = 0 end
		return math.min(apiFree, expectedFree)
	end

	function Talented:GetTalentPreviewDelta(template)
		if not template or not template.talentGroup then return 0 end
		local class = template.class
		...
		for tab, tree in ipairs(self:UncompressSpellData(class)) do
			...
			if ttab then
				for index = 1, #tree do
					local preview = ttab[index] or 0
					local learned = self:GetLearnedTalentRank(template, tab, index)
					if preview > learned then
						delta = delta + (preview - learned)
					end
				end
			end
		end
	...
	function Talented:CanAddTalentPreviewPoint(template)
		if not template or not template.talentGroup then return true end
		return self:GetTalentPreviewDelta(template) < self:GetAvailableTalentPointsForTemplate(template)
	end

	function Talented:ResetTalentPreview(template)
		if not template or not template.talentGroup then return end
		local pet = not RAID_CLASS_COLORS[template.class]
		...
		for tab, tree in ipairs(info) do
			...
			for index = 1, #tree do
				ttab[index] = select(5, GetTalentInfo(tab, index, nil, pet, group)) or 0
			end
		end
		template.points = nil
		template.talentedDirty = nil
	...
	function Talented:SnapshotTemplateEdit(template)
		if not template or template.talentGroup then return end
		local snap = {}
		for tab, ranks in ipairs(template) do
			if type(ranks) == "table" then
				snap[tab] = {}
				for index, value in ipairs(ranks) do
					snap[tab][index] = value
				end
			end
		end
		template.talentedEditSnapshot = snap
	end

	function Talented:IsTemplateEditDirty(template)
		if not template or template.talentGroup then return false end
		local snap = template.talentedEditSnapshot
		if not snap then return false end
		for tab, ranks in ipairs(template) do
			if type(ranks) == "table" then
				local sranks = snap[tab]
				if type(sranks) ~= "table" then return true end
				for index, value in ipairs(ranks) do
					if (sranks[index] or 0) ~= (value or 0) then
						return true
					end
				end
			end
	...
	function Talented:RestoreTemplateEditSnapshot(template)
		if not template or template.talentGroup then return end
		local snap = template.talentedEditSnapshot
		if not snap then return end
		for tab, ranks in pairs(snap) do
			local ttab = template[tab]
			if type(ttab) == "table" and type(ranks) == "table" then
	...
	-- True when a custom template can be learned onto the active spec without a respec:
	-- every learned rank is already part of the template, and remaining ranks fit in unspent points.
	function Talented:CanApplyTemplate(template)
		...
		if pet then
			if not self.GetPetClass or self:GetPetClass() ~= class then return false end
		elseif select(2, UnitClass("player")) ~= class then
			return false
		end

		local group = GetActiveTalentGroup(nil, pet)
		local available = self:GetAvailableTalentPointsForTemplate({
			class = class,
			talentGroup = group,
			pet = pet,
		})
		...
		for tab, tree in ipairs(info) do
			local ttab = template[tab]
			for index = 1, #tree do
				local want = (ttab and ttab[index]) or 0
				local learned = select(5, GetTalentInfo(tab, index, nil, pet, group)) or 0
				if learned > want then
					return false
				end
				need = need + (want - learned)
			end
		end

		return need > 0 and need <= available
	end

	function Talented:ConfirmApplyTemplate()
		local template = self.template
		if not template or template.talentGroup then return end
		if self.CanApplyTemplate and not self:CanApplyTemplate(template) then return end
		StaticPopupDialogs.TALENTED_CONFIRM_APPLY_TEMPLATE = {
			text = L["Are you sure you want to apply this template?"],
			button1 = YES,
			button2 = NO,
			OnAccept = function()
		...
		StaticPopup_Show("TALENTED_CONFIRM_APPLY_TEMPLATE")
	end
	...
	function Talented:UpdatePlayerSpecs()
		...
		for talentGroup = 1, GetNumTalentGroups() do
			...
			-- Keep preliminary allocation while the player is previewing
			local keepPreview = template.talentedDirty or (self.template == template and self:IsTalentPreviewDirty(template))
			if keepPreview and self:GetTalentPreviewDelta(template) > self:GetAvailableTalentPointsForTemplate(template) then
				keepPreview = false
			end
...
do
	local LAYOUT_BASE_X = 8
	local LAYOUT_BASE_Y = 24
	-- Extra width per tree column (widens the window without packing icons tighter)
	local LAYOUT_COLUMN_EXTRA = 36
	-- Gap between the 3 tree columns (no Blizzard scrollbar gutter)
	local LAYOUT_TREE_GAP = 6
	-- Single-tree (pet) windows need extra chrome width so RU toolbar labels fit
	-- (Действия / Шаблоны / Редактировать шаблон) without overlapping.
	local PET_FRAME_EXTRA_WIDTH = 160
	-- Space reserved at top of each tree for the column header bar (skin)
	local TREE_HEADER_HEIGHT = 30
	Talented.TREE_HEADER_HEIGHT = TREE_HEADER_HEIGHT
	...
	local function RecalcLayout(offset)
		if LAYOUT_OFFSET_X ~= offset then
			...
			-- Center the 4-column talent grid inside the wider tree frame
			LAYOUT_DELTA_X = LAYOUT_OFFSET_X / 2 + LAYOUT_COLUMN_EXTRA / 2
			LAYOUT_DELTA_Y = LAYOUT_OFFSET_Y / 2

			LAYOUT_SIZE_X --[[LAYOUT_MAX_COLUMNS]] = 4 * LAYOUT_OFFSET_X + LAYOUT_DELTA_X + LAYOUT_COLUMN_EXTRA / 2
	...
	local function offset(row, column)
		return (column - 1) * LAYOUT_OFFSET_X + LAYOUT_DELTA_X,
			-((row - 1) * LAYOUT_OFFSET_Y + LAYOUT_DELTA_Y + TREE_HEADER_HEIGHT)
	end
	...
	function TalentView:SetClass(class, force)
		...
		if self.frame.SetTabSize then
			...
			-- Skin provides title + toolbar chrome height; fallback = old single-row header
			if self.frame.talentedChromeHeight then
				top_offset = self.frame.talentedChromeHeight + 4
				-- Default compact footer; Update grows when points-left label shows
				bottom_offset = 10
			else
		...
		local size_y = first_tree[#first_tree].row * LAYOUT_OFFSET_Y + LAYOUT_DELTA_Y + TREE_HEADER_HEIGHT
		for tab, tree in ipairs(talents) do
			...
			local treeX = (tab - 1) * (LAYOUT_SIZE_X + LAYOUT_TREE_GAP) + LAYOUT_BASE_X
			-- Center the single pet tree inside the widened frame
			if pet and #talents == 1 then
				treeX = LAYOUT_BASE_X + PET_FRAME_EXTRA_WIDTH / 2
			end
			frame:SetPoint("TOPLEFT", treeX, -top_offset)
		end
		local gaps = math.max(#talents - 1, 0) * LAYOUT_TREE_GAP
		local bodyH = size_y + top_offset
		local frameH = bodyH + bottom_offset
		local frameW = #talents * LAYOUT_SIZE_X + gaps + LAYOUT_BASE_X * 2
		if pet then
			frameW = frameW + PET_FRAME_EXTRA_WIDTH
		end
		self.frame:SetSize(frameW, frameH)
		-- Body = chrome + trees (no footer). Update/skin grow/shrink footer on top of this.
		self.frame.talentedBodyHeight = bodyH
		self.frame.talentedSlimHeight = frameH
		self.frame.talentedFooterPad = bottom_offset
		self.frame.talentedFooterState = bottom_offset
		-- Keep skin talent-size cache in sync (pet = 1 tree, player = 3) so reopen
		-- does not restore a stale narrow width from a previous pet/glyph view.
		self.frame.talentedTalentW = self.frame:GetWidth()
		self.frame.talentedTalentH = frameH
	...
	function TalentView:Update()
		...
		local liveCanAddPoint = (not template.talentGroup) or Talented:CanAddTalentPreviewPoint(template)
		for tab, tree in ipairs(info) do
			local count = 0
			for index, talent in ipairs(tree) do
				if not talent.inactive then
					...
					local learned = template.talentGroup and Talented:GetLearnedTalentRank(template, tab, index) or 0
					-- Live specs stay interactive in view mode (preliminary allocation)
					if state == "empty" and (at_cap or (self.mode == "view" and not template.talentGroup)) then
						state = "unavailable"
					end
					if template.talentGroup and not liveCanAddPoint then
						if state == "empty" then
							state = "unavailable"
						end
					end
					...
					if req then
						...
						if ecolor == GREEN_FONT_COLOR then
							if self.mode == "edit" or template.talentGroup then
			...
			-- Live preview: allow clearing a tree back to learned ranks; templates need edit mode
			local canClear = count > 0 and (
				(template.talentGroup and Talented:IsTalentPreviewDirty(template))
				or (not template.talentGroup and self.mode == "edit")
			)
			if canClear then
				clear:Show()
			else
		...
		local maxpoints
		if template.talentGroup then
			maxpoints = Talented:GetLearnedPointCount(template) + Talented:GetAvailableTalentPointsForTemplate(template)
		else
		...
		local free = maxpoints - total
		local showPointsLeft = free > 0
		-- Footer also stays tall for Apply template / Activate button
		local showFooterAction = false
		if not template.talentGroup then
			showFooterAction = self.CanApplyTemplate and self:CanApplyTemplate(template)
		elseif not template.pet and template.talentGroup ~= GetActiveTalentGroup() then
			showFooterAction = true
		end
		local footerExpanded = showPointsLeft or showFooterAction
		self.frame.talentedShowPointsLeft = showPointsLeft
		self.frame.talentedFooterExpanded = footerExpanded
		if pointsleft then
			if showPointsLeft then
				pointsleft:Show()
				pointsleft.text:SetFormattedText(L["You have %d talent |4point:points; left"], free)
			else
				pointsleft:Hide()
			end
		end
		-- Apply footer height (points label and/or footer action button)
		do
			local frame = self.frame
			if frame.talentedActiveTab ~= 2 and frame.talentedBodyHeight and frame.talentedBodyHeight > 1 then
				local none = frame.talentedFooterNone or 10
				local pts = frame.talentedFooterPoints or 30
				local footer = footerExpanded and pts or none
				local targetH = frame.talentedBodyHeight + footer
				if frame.talentedFooterState ~= footer or math.abs((frame:GetHeight() or 0) - targetH) > 0.5 then
		...
		if cb then
			-- Edit checkbox only for custom templates (live uses Learn/Reset in the skin)
			if template.talentGroup then
				cb:Hide()
				if template.talentGroup ~= GetActiveTalentGroup() and not template.pet and activate then
	...
	function TalentView:UpdateTalent(tab, index, offset)
		...
		-- Live active specs: always allow preliminary allocation (no per-click learn)
		-- Custom templates: require edit mode
		local isLive = template.talentGroup ~= nil
		local pet = not RAID_CLASS_COLORS[template.class]
		local isActiveLive = isLive and (
			template.pet or template.talentGroup == GetActiveTalentGroup(nil, pet)
		)
		if isLive and not isActiveLive then return end
		if not isLive and self.mode ~= "edit" then return end

		if offset > 0 and Talented:IsTemplateAtCap(template) then return end
		if offset > 0 and isLive and not Talented:CanAddTalentPreviewPoint(template) then return end
		local s = Talented:GetTalentState(template, tab, index)
		if offset > 0 and s == "unavailable" then return end
		...
		local minRank = 0
		if isLive then
			-- Cannot unlearn already-learned ranks in preview; only undo free points
			minRank = Talented:GetLearnedTalentRank(template, tab, index)
		end
		if value < minRank then
			value = minRank
		...
		template.points = nil
		if isLive then
			template.talentedDirty = Talented:IsTalentPreviewDirty(template) or nil
		end
	...
	function TalentView:ClearTalentTab(t)
		...
		if template.talentGroup then
			...
			if tab then
				for index = 1, #tab do
					tab[index] = select(5, GetTalentInfo(t, index, nil, pet, template.talentGroup)) or 0
				end
			end
			template.points = nil
			template.talentedDirty = Talented:IsTalentPreviewDirty(template) or nil
		elseif not template.talentGroup then
			local tab = template[t]
		...
		if not p.always_call_learn_talents then
			...
			if
				state == "full" or -- talent maxed out
					...
					self:GetAvailableTalentPointsForTemplate(template) == 0
			 then -- no more points
				return
			end
		end
...
do
	...
	function Talented:SetTooltipInfo(frame, class, tab, index)
		...
		-- Live specs always allow preliminary allocation; templates need edit mode
		if template.talentGroup or self.mode == "edit" then
			local learned = template.talentGroup and self:GetLearnedTalentRank(template, tab, index) or 0
			local cur = template[tab] and template[tab][index] or 0
			local canAddPoint = (not template.talentGroup) or self:CanAddTalentPreviewPoint(template)
			if s == "full" and cur > learned then
			...
			elseif s == "available" then
				...
				elseif canAddPoint then
					addline(TALENT_TOOLTIP_ADDPREVIEWPOINT, GREEN_FONT_COLOR)
				end
			elseif s == "empty" and canAddPoint then
				addline(TALENT_TOOLTIP_ADDPREVIEWPOINT, GREEN_FONT_COLOR)
			end
		end
		GameTooltip:Show()
...
do
	function Talented:ApplyCurrentTemplate()
		...
		local count = 0
		local group = template.talentGroup or GetActiveTalentGroup(nil, pet)
		-- check if enough talent points are available (vs actually learned ranks)
		local available = self:GetAvailableTalentPointsForTemplate({
		...
		for tab, tree in ipairs(self:UncompressSpellData(template.class)) do
			for index = 1, #tree do
				local delta = (template[tab][index] or 0) - self:GetLearnedTalentRank(template, tab, index)
				if delta > 0 then
					count = count + delta
				end
			end
	...
	function Talented:ConfirmLearnTalentPreview()
		local template = self.template
		if not template or not self:IsTalentPreviewDirty(template) then
			return
		end
		StaticPopupDialogs.TALENTED_CONFIRM_LEARN_PREVIEW = {
			text = L["Are you sure you want to learn the selected talents?"],
			button1 = YES,
			button2 = NO,
			OnAccept = function()
				Talented:SetMode("apply")
		...
		StaticPopup_Show("TALENTED_CONFIRM_LEARN_PREVIEW")
	end
...
do
	...
	function Talented:PetTalentsAvailable()
		local talentGroup = GetActiveTalentGroup(nil, true)
		if not talentGroup then return false end
		if GetNumTalentTabs(nil, true) == 0 then return false end
		local has_talent = GetTalentInfo(1, 1, nil, true, talentGroup) or GetTalentInfo(1, 2, nil, true, talentGroup)
		return has_talent and true or false
	end

	function Talented:PET_TALENT_UPDATE()
		local class = self:GetPetClass()
		if not class or not self:PetTalentsAvailable() then
			local old = self.pet_current
			self.pet_current = nil
			if old then
				for _, view in self:IterateTalentViews(old) do
					if self:GetActiveSpec() then
						view:SetTemplate(self:GetActiveSpec())
					end
				end
			end
			return

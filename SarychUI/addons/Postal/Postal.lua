...
-- Hides the minimap unread mail button if there are no unread mail.
-- Does not scan past the first 50 items since only the first 50 are viewable.
-- Also used after Open All so the icon clears without waiting for mailbox close
-- (private servers often leave HasNewMail() stuck after mass loot).
function Postal:UpdateMinimapMailIcon()
...
-- Blizzard Shift-click: AutoLootMailItem + InboxFrame_OnClick → GetInboxText.
-- GetInboxText is what marks the mail read and clears pending-mail / HasNewMail.
-- Open All / Select that only Take*/AutoLoot leave the flag stuck across /reload.
function Postal:MarkInboxMailRead(index)
	index = tonumber(index)
	if not index or index < 1 then
		return
	end
	if index > (GetInboxNumItems() or 0) then
		return
	end
	GetInboxText(index)
end

-- Match OpenMailFrame_OnHide: empty read mail is deleted so pending-mail can clear.
-- Returns true if DeleteInboxItem was sent (caller should wait for MAIL_INBOX_UPDATE).
function Postal:DeleteInboxMailIfEmpty(index)
	index = tonumber(index)
	if not index or index < 1 or index > (GetInboxNumItems() or 0) then
		return false
	end
	self:MarkInboxMailRead(index)
	...
	local money, _, _, itemCount = select(5, GetInboxHeaderInfo(index))
	if (money or 0) ~= 0 or itemCount or isAuctionTempInvoice then
		return false
	end
...
function Postal:CountInboxAttachments(mailIndex)
	local n = 0
	for i = 1, ATTACHMENTS_MAX_RECEIVE do
		if GetInboxItemLink(mailIndex, i) then
			n = n + 1
		end
	end
	return n
...
function Postal:CountItemsAndMoney()
	-- Count real attachment slots. On some 3.3.5 private servers GetInboxHeaderInfo
	-- hasItem is just 1 for any multi-item mail — using it alone makes Open All
	-- think 1 item was taken when 3 were looted and desyncs the mail icon flag.
	local numAttach = 0
	local numGold = 0
	for i = 1, GetInboxNumItems() do
		local msgMoney = select(5, GetInboxHeaderInfo(i))
		numGold = numGold + (msgMoney or 0)
		numAttach = numAttach + self:CountInboxAttachments(i)
	end

if _G.SarychUI_ShouldLoadPostalEmbedded and not _G.SarychUI_ShouldLoadPostalEmbedded() then
	return
end
...
local wait
local waitMode -- "autoloot" | "take" | "money" | "delete"
local lastAttOnMail
local button
...
local lastAnnouncedMailIndex

local function SetOpenAllProgress(active)
	if not button then
		return
	end

	if active then
		if SarychUI and SarychUI.ShowSpinnerOn then
			button:SetText("")
			SarychUI:ShowSpinnerOn(button)
		else
			button:SetText(L["Opening..."] or "Opening...")
		end
	else
		if SarychUI and SarychUI.HideSpinner then
			SarychUI:HideSpinner()
		end
...
function Postal_OpenAll:OpenAll()
	...
	lastAnnouncedMailIndex = nil
	if mailIndex == 0 then
		return
	end

	Postal:DisableInbox(1)
	SetOpenAllProgress(true)
...
local PendingDel
-- Wake as soon as the server updates the inbox (same signal Blizzard uses after loot).
...
function Postal_OpenAll:ProcessNext()
	if mailIndex > 0 then
		...
		-- Wait until THIS mail is actually finished — not "any inbox counter changed".
		-- Re-firing AutoLootMailItem on the first slot update is what desyncs HasNewMail
		-- vs default Shift-click (one AutoLoot, then done).
		if wait then
			local numMails = GetInboxNumItems() or 0
			if mailIndex > numMails then
				-- Mail auto-deleted after full loot
				wait = false
				waitMode = nil
				mailIndex = numMails
				attachIndex = ATTACHMENTS_MAX_RECEIVE
			...
			if waitMode == "autoloot" then
				-- One AutoLootMailItem call — wait until ALL slots on this mail are gone
				if Postal:CountInboxAttachments(mailIndex) > 0 then
					updateFrame:Show()
					return
				end
				wait = false
				...
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext()
			elseif waitMode == "take" then
				-- Single TakeInboxItem — wait until this mail's slot count drops, then take next
				local left = Postal:CountInboxAttachments(mailIndex)
				if left >= (lastAttOnMail or 0) and left > 0 then
					updateFrame:Show()
					return
				end
				wait = false
				...
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext()
			elseif waitMode == "money" then
				local moneyLeft = select(5, GetInboxHeaderInfo(mailIndex)) or 0
				if moneyLeft > 0 then
					updateFrame:Show()
					return
				end
				wait = false
				waitMode = nil
				-- After gold: mark read + delete empty like closing OpenMail
				if Postal:DeleteInboxMailIfEmpty(mailIndex) then
					wait = true
					waitMode = "delete"
					updateFrame:Show()
					return
				...
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext()
			elseif waitMode == "delete" then
				-- DeleteInboxItem in flight; mailIndex > numMails handled above
				local moneyLeft = select(5, GetInboxHeaderInfo(mailIndex)) or 0
				local attLeft = Postal:CountInboxAttachments(mailIndex)
				if moneyLeft == 0 and attLeft == 0 then
					updateFrame:Show()
					return
				end
				wait = false
		...
		-- Verbose: one line per mail with subject (classic Postal)
		if Postal.db.profile.OpenAll.SpamChat and lastAnnouncedMailIndex ~= mailIndex then
			lastAnnouncedMailIndex = mailIndex
			local moneyString = (msgMoney and msgMoney > 0) and (" [" .. Postal:GetMoneyString(msgMoney) .. "]") or ""
		...
		if attachIndex > 0 and not invFull then
			-- Same API as Blizzard Shift-click: AutoLoot then mark read (GetInboxText).
			-- Loot alone does not clear HasNewMail — opening the mail does.
			if type(AutoLootMailItem) == "function" then
				AutoLootMailItem(mailIndex)
				waitMode = "autoloot"
			else
				lastAttOnMail = Postal:CountInboxAttachments(mailIndex)
				TakeInboxItem(mailIndex, attachIndex)
				waitMode = "take"
			end
			Postal:MarkInboxMailRead(mailIndex)
			wait = true
			updateFrame:Show()
		elseif msgMoney and msgMoney > 0 then
			...
			-- Empty of loot: mark read + delete like closing OpenMailFrame
			if Postal:DeleteInboxMailIfEmpty(mailIndex) then
				wait = true
				waitMode = "delete"
				updateFrame:Show()
...
function Postal_OpenAll:Reset(event)
	...
	SetOpenAllProgress(false)
	Postal:DisableInbox()
	InboxFrame_Update()
	-- Same as closing the mailbox: clear minimap mail icon when nothing unread remains.
	if Postal.UpdateMinimapMailIcon then
		Postal:UpdateMinimapMailIcon()
	end

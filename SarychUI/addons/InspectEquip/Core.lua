...
InspectEquipEnabled = InspectEquipEnabled -- set by SarychUI bootstrap/wrapper
...
local function IsInspectEquipRuntimeEnabled()
	if InspectEquipEnabled == false then
		return false
	end
	if SarychUI_IsStandaloneInspectEquipEnabled and SarychUI_IsStandaloneInspectEquipEnabled() then
		return false
	end
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.InspectEquip then
		return SarychUI.db.profile.addons.InspectEquip.enabled == true
	end
	return InspectEquipEnabled ~= false
end
...
function IE.RegisterInterfaceOptions()
  if IE._suiBlizRegistered then
    return
  end
  LibStub("AceConfigDialog-3.0"):AddToBlizOptions("InspectEquip")
  IE._suiBlizRegistered = true
end

--------------------------------------------------------------------------------------

IE.defaultEnabledState = false
if IE.SetEnabledState then
  IE:SetEnabledState(false)
end
...
function IE:OnEnable()
  if not IsInspectEquipRuntimeEnabled() then
    return
  end
...
function IE:InspectUnit(unit, ...)
  ...
  if SarychUI_PerfLog then
    SarychUI_PerfLog("InspectEquip", "InspectUnit", unit)
  end
  ...
  if SarychUI_PerfSlow then
    SarychUI_PerfSlow("InspectEquip", "InspectUnitSlow", perfStart, unit, 3)
  end
end
...
function IE:UNIT_INVENTORY_CHANGED(event, unit)
  if SarychUI_PerfLog then
    SarychUI_PerfLog("InspectEquip", "UNIT_INVENTORY_CHANGED", unit)
  end
...
function IE:Inspect(unit, entry)
  local perfStart = SarychUI_PerfDebug and SarychUI_PerfNow and SarychUI_PerfNow() or nil
  if SarychUI_PerfLog then
    SarychUI_PerfLog("InspectEquip", "Inspect", unit)
  end
  ...
  if SarychUI_PerfSlow then
    SarychUI_PerfSlow("InspectEquip", "InspectSlow", perfStart, unit, 3)
  end
end

...
local function IsEnabled()
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.Mapster then
		return SarychUI.db.profile.addons.Mapster.enabled ~= false;
	end
...
function Mapster:OnInitialize()
	...
	-- Register Mapster in global namespace so other addons can detect it
	-- This makes _G.Mapster available and IsAddOnLoaded("Mapster") work
	if not _G.Mapster then
		_G.Mapster = self
	end
	
	-- Create a secure hook for IsAddOnLoaded to make Mapster detectable
	-- We use hooksecurefunc to avoid conflicts with other addons
	local originalIsAddOnLoaded = _G.IsAddOnLoaded
	if originalIsAddOnLoaded then
		-- Store original function
		local _originalIsAddOnLoaded = originalIsAddOnLoaded
		
		-- Create wrapper that checks for Mapster first
		_G.IsAddOnLoaded = function(addon)
			if addon == "Mapster" and IsEnabled() then
				return true
			end
			return _originalIsAddOnLoaded(addon)
		end
	end
	
	-- Create a secure hook for GetAddOnInfo to make Mapster detectable
	local originalGetAddOnInfo = _G.GetAddOnInfo
	if originalGetAddOnInfo then
		-- Store original function
		local _originalGetAddOnInfo = originalGetAddOnInfo
		SarychUI_MapsterGetAddOnInfo = _originalGetAddOnInfo
		
		-- Create wrapper that checks for Mapster first
		_G.GetAddOnInfo = function(addon, index)
			if addon == "Mapster" and IsEnabled() then
				return "Mapster", "Mapster", "Simple Map Mod", true, true, nil, "INSECURE"
			end
			if index then
				return _originalGetAddOnInfo(index)
			else
				return _originalGetAddOnInfo(addon)
			end
		end
	end

	-- Embedded in SarychUI: defer Ace enable until wrapper reads db on ADDON_LOADED.
	if SarychUI then
...
function Mapster:OnEnable()
	...
	-- WorldMapFrame must not remain in UIPanel slots after we detach it from UIPanelWindows.
	if GetUIPanel then
	...
	if vis then
		-- UIPanel layout is disabled; ShowUIPanel would corrupt UpdateUIPanelPositions.
		WorldMapFrame:Show()
	end
	self:LoadFogClearSettings()
end

function Mapster:LoadFogClearSettings()
    local fogClear = self:GetModule("FogClear", true)
    if fogClear then
        fogClear.db.profile.colorR = self.db.profile.FogClear.colorR
        fogClear.db.profile.colorB = self.db.profile.FogClear.colorB
        fogClear.db.profile.colorA = self.db.profile.FogClear.colorA
        fogClear.db.profile.colorG = self.db.profile.FogClear.colorG
        fogClear:Refresh()
    end
end
...
function Mapster:UpdateMapElements()
	...
	if self.elementsHidden and (mouseOver or not db.hideBorder) then
		if SarychUI_PerfLog then
			SarychUI_PerfLog("Mapster", "MapElementsShow", "mouseover")
		end
	...
	elseif not self.elementsHidden and not mouseOver and db.hideBorder then
		if SarychUI_PerfLog then
			SarychUI_PerfLog("Mapster", "MapElementsHide", "no-mouseover")
		end
	...
	if SarychUI_PerfSlow then
		SarychUI_PerfSlow("Mapster", "UpdateMapElementsSlow", perfStart, nil, 3)
	end
end

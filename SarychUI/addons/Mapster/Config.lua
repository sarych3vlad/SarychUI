...
local function optFunc()
	...
	if not Mapster.optionsFrames or not Mapster.optionsFrames.Mapster then
		return
	end
...
local moduleBlizNames = {}

function Mapster:SetupOptions()
	self.optionsFrames = {}

	-- setup options table (Blizzard category deferred until first enable)
...
function Mapster:RegisterModuleOptions(name, optionTbl, displayName)
	...
	moduleBlizNames[name] = displayName
	if self._suiBlizRegistered then
		self.optionsFrames = self.optionsFrames or {}
...
function Mapster:RegisterInterfaceOptions()
	if self._suiBlizRegistered then
		return
	end
	self.optionsFrames = self.optionsFrames or {}
	self.optionsFrames.Mapster = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Mapster", nil, nil, "general")
	if moduleBlizNames.Profiles then
		self.optionsFrames.Profiles = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Mapster", moduleBlizNames.Profiles, "Mapster", "Profiles")
	end
	for name, displayName in pairs(moduleBlizNames) do
		if name ~= "Profiles" and not self.optionsFrames[name] then
			self.optionsFrames[name] = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Mapster", displayName, "Mapster", name)
		end
	end

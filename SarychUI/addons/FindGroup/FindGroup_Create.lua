...
function FGC_Icons_settext(f, class, sh, role, num)
		...
		texture2:SetTexture("Interface\\AddOns\\SarychUI\\addons\\FindGroup\\textures\\ICONS-CLASSES-GRAY");
		texture2:SetTexCoord(coords[1], coords[2], coords[3], coords[4]);
		f:SetNormalTexture(texture2)

		local texture3 = f:CreateTexture()
		texture3:SetTexture("Interface\\AddOns\\SarychUI\\addons\\FindGroup\\textures\\ICONS-CLASSES-GRAY");
...
hooksecurefunc("UIDropDownMenu_OnHide", function()
	-- Only wipe custom textures for FindGroup's own menus. Clearing DropDownList
	-- button textures globally blanks other addons' Blizzard dropdowns.
	local fgOpen = (FindGroupFrame and FindGroupFrame:IsShown())
		or (FindGroupOptionsFrame and FindGroupOptionsFrame:IsShown())
		or (FindGroupShadow and FindGroupShadow:IsShown())
	if not fgOpen then
		return
	end
	if not (SarychUI_FindGroupIsActive and SarychUI_FindGroupIsActive()) then
		return
	end

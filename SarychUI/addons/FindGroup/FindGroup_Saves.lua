...
local function FGSaveActive()
	return SarychUI_FindGroupIsActive and SarychUI_FindGroupIsActive()
end
...
mynewframe_i:SetScript("OnEvent", function()
	if not FGSaveActive() or not PlAYER_ONLINE_MENU then
		return
	end
...
local savesChatFiltersRegistered = false

local SAVES_CHANNEL_FILTER_EVENTS = {
...
function SarychUI_FindGroup_RegisterSavesChatFilters()
	if savesChatFiltersRegistered then
		return
	end
	ChatFrame_AddMessageEventFilter("CHAT_MSG_SYSTEM", myChatFilter)
	for i = 1, #SAVES_CHANNEL_FILTER_EVENTS do
		ChatFrame_AddMessageEventFilter(SAVES_CHANNEL_FILTER_EVENTS[i], my_channel_filter)
	end
	savesChatFiltersRegistered = true
end

function SarychUI_FindGroup_UnregisterSavesChatFilters()
	if not savesChatFiltersRegistered then
		return
	end
	ChatFrame_RemoveMessageEventFilter("CHAT_MSG_SYSTEM", myChatFilter)
	for i = 1, #SAVES_CHANNEL_FILTER_EVENTS do
		ChatFrame_RemoveMessageEventFilter(SAVES_CHANNEL_FILTER_EVENTS[i], my_channel_filter)
	end
	savesChatFiltersRegistered = false
end

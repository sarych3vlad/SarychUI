...
local findChatFiltersRegistered = false

local FIND_CHAT_FILTER_EVENTS = {
	"CHAT_MSG_OFFICER",
	"CHAT_MSG_CHANNEL",
	"CHAT_MSG_GUILD",
	"CHAT_MSG_YELL",
...
function SarychUI_FindGroup_RegisterFindChatFilters()
	if findChatFiltersRegistered then
		return
	end
	for i = 1, #FIND_CHAT_FILTER_EVENTS do
		ChatFrame_AddMessageEventFilter(FIND_CHAT_FILTER_EVENTS[i], my_channel_filter)
	end
	findChatFiltersRegistered = true
end

function SarychUI_FindGroup_UnregisterFindChatFilters()
	if not findChatFiltersRegistered then
		return
	end
	for i = 1, #FIND_CHAT_FILTER_EVENTS do
		ChatFrame_RemoveMessageEventFilter(FIND_CHAT_FILTER_EVENTS[i], my_channel_filter)
	end
	findChatFiltersRegistered = false
end

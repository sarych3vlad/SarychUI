...
-- NOTE: aura display over nameplates has been intentionally removed from this
-- standalone addon, so LibAuraInfo (LAI) is no longer used or loaded.
...
local format, gsub, match, split, find = string.format, string.gsub, string.match, string.split, string.find
...
local function IsNamePlatesRuntimeActive()
	if not NP.Initialized then
		return false
	end
	if E.IsNamePlatesRuntimeEnabled then
		return E:IsNamePlatesRuntimeEnabled()
	end
	return E.private and E.private.nameplates and E.private.nameplates.enable == true
end
...
function NP:SetFrameScale(frame, scale, noPlayAnimation)
	if frame.currentScale ~= scale then
		...
		if self:ShouldSyncNameToHealthWidth(frame) then
...
function NP:OnShow(isConfig, dontHideHighlight)
	if not IsNamePlatesRuntimeActive() then
		return
	end
...
function NP:ConfigureAll()
	...
	self:EnsureNameAbbreviationDefaults()
...
-- Extra clickable height so the name above the health bar stays inside the hit rect.
function NP:GetClickableNamePad(unitFrame)
	if not unitFrame or not unitFrame.UnitType or not self.db then
		return 0
	end

	local faction = (unitFrame.UnitType == "FRIENDLY_PLAYER" or unitFrame.UnitType == "FRIENDLY_NPC") and "friendly" or "enemy"
	local plateSize = self.db.plateSize
	local includeName = faction == "friendly" and plateSize.friendlyIncludeName or plateSize.enemyIncludeName
	if not includeName then
		return 0
	end

	local unitDB = self.db.units[unitFrame.UnitType]
	if not unitDB then
		return 0
	end

	local nameDB = unitDB.name
	if not nameDB or not nameDB.enable then
		return 0
	end

	local fontSize = nameDB.fontSize or self.db.fontSize or 11
	local yOffset = nameDB.yOffset or 0
	local healthEnabled = unitDB.health and unitDB.health.enable
	if not healthEnabled and self.db.alwaysShowTargetHealth and unitFrame.isTarget then
		healthEnabled = true
	end

	if healthEnabled then
		local position = nameDB.position or ""
		-- Only pad upward when the name is anchored above the health bar.
		if not find(position, "TOP", 1, true) then
			return 0
		end
		return fontSize + max(0, yOffset) + 2
	end

	-- Name-only plates: keep glyph tops inside the clickable frame.
	return fontSize + 2
end
...
function NP:SetSize(frame)
	if InCombatLockdown() then
		...
		if self.db.clickThrough[unitType] then
			...
			local pad = self:GetClickableNamePad(unitFrame)
			if unitType == "friendly" then
				frame:SetSize(self.db.plateSize.friendlyWidth, self.db.plateSize.friendlyHeight + pad)
			else
				frame:SetSize(self.db.plateSize.enemyWidth, self.db.plateSize.enemyHeight + pad)
			end
		end
...
function NP:OnCreated(frame)
	...
	local unitFrame = CreateFrame("Frame", format("ENP_NamePlate%d", plateID), frame)
	...
	self:HookHealthBarNameSync(unitFrame.Health, unitFrame)
	...
	if unitFrame.oldCastBar then
		...
		unitFrame.oldCastBar.Border = CastBarBorder
	end
	...
	-- Use the client's default nameplate castbar as the castbar trigger.
	self:HookDefaultCastBar(unitFrame)
...
function NP:UnregisterCastEvents(frame)
...
function NP:SetTargetFrame(frame)
	...
	elseif frame.isTarget then
		...
		if not frame.isGroupUnit then
			...
			if frame.isEventsRegistered then
				self:UnregisterCastEvents(frame)
...
function NP:OnUpdate()
	...
	if lastChildern ~= numChildren then
		if SarychUI_PerfLog then
			SarychUI_PerfLog("ElvUI_NamePlates", "WorldFrameChildren", tostring(lastChildern) .. "->" .. tostring(numChildren))
		end
	...
	if SarychUI_PerfSlow then
		SarychUI_PerfSlow("ElvUI_NamePlates", "OnUpdateSlow", perfStart, nil, 3)
	end
end
...
function NP:UpdateCVars()
	...
	-- Keep the default nameplate castbar enabled: we use it as the trigger source
	-- for the ElvUI castbar (patched client shows it under all casting nameplates).
...
function NP:TogleTestFrame(unitType)
	local unitFrame = ENP_Test.UnitFrame
	if not ENP_Test:IsShown() or unitFrame.UnitType ~= unitType then
...
function NP:Initialize()
	...
	local ENP_Test = CreateFrame("Button", "ENP_Test")
	ENP_Test:Point("BOTTOM", UIParent, "BOTTOM", 0, 250)
	ENP_Test:SetMovable(true)
	ENP_Test:RegisterForDrag("LeftButton", "RightButton")
	ENP_Test:SetScript("OnDragStart", function() ENP_Test:StartMoving() end)
	ENP_Test:SetScript("OnDragStop", function() ENP_Test:StopMovingOrSizing() end)

	CreateFrame("StatusBar", nil, ENP_Test)
	CreateFrame("StatusBar", nil, ENP_Test)

	for i = 1, 11 do
		if i == 7 or i == 8 then
			ENP_Test:CreateFontString(nil, "OVERLAY", "GameFontNormal"):SetText("Empty")
		else
			ENP_Test:CreateTexture():Hide()
		end
	end

	self:StyleFrame(ENP_Test, true)
	self:OnCreated(ENP_Test)
	local castbar = ENP_Test.UnitFrame.CastBar
	castbar.__ENPTest = true
	castbar:SetParent(ENP_Test.UnitFrame.Health)
	...
	-- Aura tracking (LibAuraInfo callbacks) removed: this addon never shows auras.
end

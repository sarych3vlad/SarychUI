...
function NP:EnsureNameAbbreviationDefaults()
	if not self.db or not self.db.units then return end

	for unitType in pairs(NAME_ABBREV_UNITS) do
		local nameDB = self.db.units[unitType] and self.db.units[unitType].name
		if nameDB then
			if nameDB.noAbbreviation == nil then
				nameDB.noAbbreviation = false
			end
			if nameDB.noAbbreviationMaxChars == nil then
				nameDB.noAbbreviationMaxChars = 30
			end
		end
	end
end

function NP:GetNameAvailableWidth(frame)
	local db = self.db.units[frame.UnitType]
	local scale = frame.currentScale or 1
	local width

	local healthShown = db.health.enable or (self.db.alwaysShowTargetHealth and frame.isTarget)
	if healthShown and db.name.parent == "Health" and frame.Health then
		width = frame.Health:GetWidth()
		if not width or width <= 0 then
			width = db.health.width * scale
		end
	else
		width = db.health.width * scale
	end

	width = width - math.abs(db.name.xOffset or 0) - 4

	if db.level and db.level.enable and db.level.parent == db.name.parent then
		width = width - 24
	end
...
function NP:TruncateNameWithoutAbbreviation(frame, nameText)
	local nameDB = self.db.units[frame.UnitType].name
	local nameFS = frame.Name
	local maxChars = nameDB.noAbbreviationMaxChars or 30
	local availableWidth = self:GetNameAvailableWidth(frame)
	local totalLen = utf8len(nameText)

	if totalLen == 0 then
		return nameText
	end

	local tryLen = min(maxChars, totalLen)
	for len = tryLen, 1, -1 do
		local candidate
		if len >= totalLen then
			candidate = nameText
		else
			candidate = E:ShortenString(nameText, len, true)
		end

		nameFS:SetText(candidate)
		if nameFS:GetStringWidth() <= availableWidth then
			return candidate
		end
	end

	return E:ShortenString(nameText, 1, true)
end

function NP:FormatNameText(frame, nameText)
	local nameDB = self.db.units[frame.UnitType].name
	if not nameDB.abbrev then
		return nameText
	end

	if nameDB.noAbbreviation then
		return self:TruncateNameWithoutAbbreviation(frame, nameText)
	end

	return abbrev(nameText)
end

function NP:ShouldSyncNameToHealthWidth(frame)
	local nameDB = self.db.units[frame.UnitType] and self.db.units[frame.UnitType].name
	return nameDB and nameDB.enable and nameDB.abbrev and nameDB.noAbbreviation
end

function NP:HookHealthBarNameSync(healthBar, unitFrame)
	if not healthBar or healthBar.enpNameWidthSync then return end
	healthBar.enpNameWidthSync = true

	local originalOnSizeChanged = healthBar:GetScript("OnSizeChanged")
	healthBar:SetScript("OnSizeChanged", function(bar, width, height)
		if originalOnSizeChanged then
			originalOnSizeChanged(bar, width, height)
		end

		local frame = bar:GetParent()
		if frame and NP:ShouldSyncNameToHealthWidth(frame) then
...
function NP:Configure_Name(frame)
	...
	if frame.Health then
		self:HookHealthBarNameSync(frame.Health, frame)
	end
end

...
Graphical Button.
-------------------------------------------------------------------------------]]
-- Version 3 overrides Details' copy, which requires global ElvUI for drag-drop skinning.
...
local function Constructor()
	local name = "AceGUIENPButtonElvUI" .. AceGUI:GetNextWidgetNum(Type)
	...
	if not dragdropButton then
		dragdropButton = CreateFrame("Button", "AceGUIENPDragDropButton", UIParent, "UIPanelButtonTemplate")
		...
		dragdropButton:Hide()
		local elv = _G.ElvUI or _G.SarychUI_ElvUI_NamePlates
		if elv and elv[1] then
			local skins = elv[1]:GetModule("Skins", true)
			if skins then
				skins:HandleButton(dragdropButton)
			end
		end
	end

...
-- ENP ships this widget so Nameplates options don't depend on Details registering it.
...
-- GLOBALS: ColorPickerFrame, OpacitySliderFrame, ColorPPDefault, LibStub
...
local function ColorSwatch_OnClick(frame)
	...
	local AceGUI = LibStub and LibStub("AceGUI-3.0-ENP", true)
	if AceGUI then AceGUI:ClearFocus() end
end
...
local function Constructor()
	local AceGUI = LibStub("AceGUI-3.0-ENP")
...
local function RegisterColorPickerElvUI()
	local AceGUI = LibStub and LibStub("AceGUI-3.0-ENP", true)
	if not AceGUI then return end
	-- Re-register if missing from the shared registry (AceGUI upgrade races).
	if AceGUI.WidgetRegistry and AceGUI.WidgetRegistry[Type]
		and (AceGUI:GetWidgetVersion(Type) or 0) >= Version then
		return
	end
	AceGUI:RegisterWidgetType(Type, Constructor, Version)
end

RegisterColorPickerElvUI()
_G.SarychUI_ENP_RegisterColorPickerElvUI = RegisterColorPickerElvUI

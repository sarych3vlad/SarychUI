...
-- ENP options use an isolated AceGUI major so WeakAuras/etc. stay untouched.
local gui = LibStub("AceGUI-3.0-ENP", true) or LibStub("AceGUI-3.0")
		...
		if not hidden then
			if v.type == "group" then
				...
				elseif v.type == "color" then
					...
					-- ColorPicker-ElvUI may be missing if Details/etc didn't register it (AceGUI upgrade race).
					if not control then
						control = gui:Create("ColorPicker")
					end
					if not control then
						geterrorhandler()("AceConfigDialog-ElvUI: failed to create ColorPicker control")
						-- skip this option rather than hard-crash FeedGroup
					else
					control:SetLabel(name)

...
local AddOnName = ... or "_Cursor";
local Version = ( GetAddOnMetadata( AddOnName, "Version" ) or "3.3.0.2" ):match( "^([%d.]+)" );
_CursorEnabled = _CursorEnabled -- set by SarychUI bootstrap/wrapper (false until enabled)
-- Defaults before SavedVariables overwrite; keep table shape if SV already present.
_CursorOptions = _CursorOptions;
if type( _CursorOptionsCharacter ) ~= "table" then
	_CursorOptionsCharacter = {
		Cursors = {};
		Version = Version;
	};
elseif type( _CursorOptionsCharacter.Cursors ) ~= "table" then
	_CursorOptionsCharacter.Cursors = {};
end
...
do
	...
	function me:ADDON_LOADED ( _, AddOn )
		if ( AddOn:lower() == AddOnName:lower() or AddOn == "SarychUI" ) then

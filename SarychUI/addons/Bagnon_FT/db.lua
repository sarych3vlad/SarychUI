...
-- Initialize after SarychUI loads
...
--locals
local currentPlayer --the name of the current player that's logged on
local currentRealm --what currentRealm we're on
local playerList --a sorted list of players

-- Function to update current player and realm
local function UpdatePlayerInfo()
	currentPlayer = UnitName('player')
	currentRealm = GetRealmName()
	playerList = nil -- Reset player list when player changes
end
...
function BagnonDB:UpdateVersion()
	BagnonForeverDB.version = CURRENT_VERSION
	print(format('BagnonFT: Updated to v%s', BagnonForeverDB.version))
end
...
function BagnonDB:PLAYER_LOGIN()
	...
	-- Update player info on login
	UpdatePlayerInfo()
	
	-- Reload settings to ensure we have the correct player data
	self:LoadSettings()

-- BNetToast Wrapper for SarychUI
-- Manages loading and enabling/disabling of BNetToast addon

local ADDON_NAME = "BNetToast"
local addonPath = "Interface\\AddOns\\SarychUI\\addons\\BNetToast\\"

-- Create wrapper module
local wrapper = {
	name = ADDON_NAME,
	author = "Bonho, Bunny67",
	version = "1.0",
	enabled = false,
	loaded = false,
}
...
BNetToastEnabled = BNetToastEnabled or true;
...
function wrapper:Initialize()
	...
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons then
		if SarychUI.db.profile.addons[ADDON_NAME] and SarychUI.db.profile.addons[ADDON_NAME].enabled then
			...
			BNetToastEnabled = true
			self.enabled = true
			self.loaded = true
		else
			-- Set global enabled state
			BNetToastEnabled = false
			self.enabled = false
		end
	else
...
function wrapper:Enable()
	...
	-- Re-register events if frame exists
	if BNetFrame then
...
function wrapper:Disable()
	...
	-- Unregister events when disabled
	if BNetFrame then
		BNetFrame:UnregisterEvent("CHAT_MSG_SYSTEM")
	end
	
	-- Hide toast frame when disabled
	...
	return {
		...
		args = {
			enabled = {
				type = "toggle",
				name = "Включить",
				desc = "Включить/выключить BNetToast",
				order = 1,
				get = function()

...
function MagicButton_OnLoad(self)
	...
	for i=1, self:GetNumPoints() do
		...
		if (relativeTo:GetObjectType() == "Button" and (point == "TOPLEFT" or point == "LEFT")) then
			...
			self.LeftSeparator:SetTexture("Interface\\AddOns\\SarychUI\\addons\\!!!ClassicAPI\\Texture\\FrameGeneral\\UI-Frame");
		...
		elseif (relativeTo:GetObjectType() == "Button" and (point == "TOPRIGHT" or point == "RIGHT")) then
			...
			self.RightSeparator:SetTexture("Interface\\AddOns\\SarychUI\\addons\\!!!ClassicAPI\\Texture\\FrameGeneral\\UI-Frame");
...
function ButtonFrameTemplate_HidePortrait(self)
	local name = self:GetName()
	local portrait = self.portrait or _G[name .. "Portrait"]
	local portraitFrame = self.portraitFrame or self.PortraitFrame or _G[name .. "PortraitFrame"]
	local topLeftCorner = self.topLeftCorner or self.TopLeftCorner or _G[name .. "TopLeftCorner"]
	local topBorderBar = self.topBorderBar or self.TopBorder or _G[name .. "TopBorder"]
	local leftBorderBar = self.leftBorderBar or self.LeftBorder or _G[name .. "LeftBorder"]
	...
	if topLeftCorner then
		topLeftCorner:Show()
		if topBorderBar then
			topBorderBar:SetPoint("TOPLEFT", topLeftCorner, "TOPRIGHT", 0, 0)
		end
		if leftBorderBar then
			leftBorderBar:SetPoint("TOPLEFT", topLeftCorner, "BOTTOMLEFT", 0, 0)
		end
	end
end
...
function ButtonFrameTemplate_ShowPortrait(self)
	...
	if portraitFrame then
		portraitFrame:Show()
		if topBorderBar then
			topBorderBar:SetPoint("TOPLEFT", portraitFrame, "TOPRIGHT", 0, -10)
		end
		if leftBorderBar then
			leftBorderBar:SetPoint("TOPLEFT", portraitFrame, "BOTTOMLEFT", 8, 0)
		end
	end
	if topLeftCorner then
		topLeftCorner:Hide()

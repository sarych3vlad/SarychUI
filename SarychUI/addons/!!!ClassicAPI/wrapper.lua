-- !!!ClassicAPI Wrapper for SarychUI
-- Manages loading and enabling/disabling of !!!ClassicAPI addon

local ADDON_NAME = "!!!ClassicAPI"
local addonPath = "Interface\\AddOns\\SarychUI\\addons\\!!!ClassicAPI\\"
...
ClassicAPIEnabled = ClassicAPIEnabled or true;
...
local function GetRuntimeEnabledFromDB()
	...
	return ClassicAPIEnabled ~= false
end
...
function wrapper:Initialize()
	...
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons then
		if SarychUI.db.profile.addons[ADDON_NAME] and SarychUI.db.profile.addons[ADDON_NAME].enabled then
			...
			ClassicAPIEnabled = true
			self.enabled = true
			self.loaded = true
		else
			-- Set global enabled state
			ClassicAPIEnabled = false
			self.enabled = false
		end
	else
...
function wrapper:Enable()
	...
	-- !!!ClassicAPI is a library addon, so it's always active when loaded
	self.enabled = true
	self.loaded = true
...
function wrapper:Disable()
	...
	-- Note: !!!ClassicAPI is a library addon, so disabling it doesn't really unload it
	-- The library will remain loaded but inactive
	self.enabled = false
	...
	return {
		...
		args = {
			enabled = {
				type = "toggle",
				name = "Включить",
				desc = "Включить/выключить !!!ClassicAPI",
				order = 1,
				get = function()
			...
			description = {
				type = "description",
				name = "Библиотека для эмуляции аспектов Classic API. Переключатель runtime-флага; файлы библиотеки остаются загруженными (Library to emulate aspects of the Classic API).\n\nАвтор: " .. (self.author or "Неизвестен") .. "\nВерсия: " .. (self.version or "Неизвестна"),
				order = 2,
				width = "full",
			},
		},

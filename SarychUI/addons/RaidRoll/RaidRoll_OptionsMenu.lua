...
function RaidRoll_RegisterInterfaceOptions()
	if RaidRoll_SuiBlizRegistered then
		return
	end
	if not RaidRoll_Panel or not RaidRoll_Panel.panel then
		return
	end
	InterfaceOptions_AddCategory(RaidRoll_Panel.panel)
	if RaidRoll_PriorityPanel then
		InterfaceOptions_AddCategory(RaidRoll_PriorityPanel)
	end
	if RaidRoll_LootTrackerLoaded == true and RaidRoll_LootPanel then
		InterfaceOptions_AddCategory(RaidRoll_LootPanel)
	end
	RaidRoll_SuiBlizRegistered = true
end
...
function Setup_RR_Panel()
 ...
 -- Blizzard Interface Options: register now only if SarychUI db already says enabled;
 -- otherwise the wrapper calls RaidRoll_RegisterInterfaceOptions() on enable.
 if RaidRollEnabled ~= false and SarychUI and SarychUI.db and SarychUI.db.profile then
	local db = SarychUI.db.profile.addons and SarychUI.db.profile.addons.RaidRoll

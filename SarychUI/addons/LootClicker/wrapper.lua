-- LootClicker wrapper for SarychUI
...
local function IsStandaloneLootClickerEnabled()
	if SarychUI_IsStandaloneLootClickerEnabled then
		return SarychUI_IsStandaloneLootClickerEnabled()
	end
	if not GetAddOnInfo then return false end
	local _, _, _, enabled, loadable = GetAddOnInfo("LootClicker")
	return loadable and enabled and true or false
end

local function ApplyLootClickerRuntime(enable)
	LootClickerEnabled = enable and true or false
	if enable then
		if LootClicker_RegisterInterfaceOptions then
			LootClicker_RegisterInterfaceOptions()
		end
		if LootClicker and LootClicker.OnEnable then
			LootClicker:OnEnable()
		end
	elseif LootClicker and LootClicker.OnDisable then
...
function wrapper:IsRuntimeEnabled()
	if IsStandaloneLootClickerEnabled() then
		return false
	end
...
function wrapper:SetRuntimeEnabled(enable)
	if IsStandaloneLootClickerEnabled() then
		if SarychUI and SarychUI.Print then
			SarychUI:Print("|cffff0000Обнаружена standalone-версия LootClicker.|r Отключите её в списке аддонов, чтобы использовать встроенную версию SarychUI.")
		end
		return false
	end
	...
	ApplyLootClickerRuntime(enable)
	if not enable then
...
local function OpenLootClickerInterfaceOptions()
	ApplyLootClickerRuntime(true)
	local category = _G.LootClickerConfig
...
function SarychUI_OpenLootClickerConfig()
	...
	if IsStandaloneLootClickerEnabled() then
		if SarychUI and SarychUI.Print then
			SarychUI:Print("|cffff0000Обнаружена standalone-версия LootClicker.|r Отключите её в списке аддонов.")
		end
		return false
	end
	...
	deferFrame:SetScript("OnUpdate", function(self)
		...
		if not OpenLootClickerInterfaceOptions() then
			if SarychUI and SarychUI.Print then
				SarychUI:Print("|cffff8080LootClicker:|r настройки недоступны. Проверьте Lua errors.")
			end
		end
	end)
	return true
...
function wrapper:OpenConfig()
	return SarychUI_OpenLootClickerConfig()
end
...
function wrapper:GetOptions()
	return {
		...
		args = {
			...
			open = {
				type = "execute",
				name = "Открыть настройки LootClicker",
				desc = "Открыть окно настроек LootClicker",
				order = 2,
				disabled = function()
			...
			description = {
				...
				name = "Автоматически нажимает кнопки при розыгрыше добычи.\nВключение — сразу; выключение из списка Модификаций — после /reload.\n\nАвтор: "
					...
					.. "\n\n|cFFFFD700Команды:|r /lootclicker или /lc",
				order = 3,
				width = "full",
			},
			conflict = {

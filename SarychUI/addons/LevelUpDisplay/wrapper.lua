-- LevelUpDisplay Wrapper for SarychUI
-- Manages loading and enabling/disabling of LevelUpDisplay addon

local ADDON_NAME = "LevelUpDisplay"
local addonPath = "Interface\\AddOns\\SarychUI\\addons\\LevelUpDisplay\\"
...
local function GetRuntimeEnabledFromDB()
	...
	return LevelUpDisplayEnabled ~= false
end
...
function wrapper:Initialize()
	...
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons then
		if SarychUI.db.profile.addons[ADDON_NAME] and SarychUI.db.profile.addons[ADDON_NAME].enabled then
			...
			LevelUpDisplayEnabled = true
			self.enabled = true
			self.loaded = true
		else
			-- Set global enabled state
			LevelUpDisplayEnabled = false
			self.enabled = false
		end
	else
...
function wrapper:Disable()
	...
	-- Hide all level up frames when disabled
	if LevelUpDisplayFrame then
		LevelUpDisplayFrame:Hide()
	end
end
	...
	return {
		...
		args = {
			enabled = {
				type = "toggle",
				name = "Включить",
				desc = "Включить/выключить LevelUpDisplay",
				order = 1,
				get = function()
			...
			description = {
				type = "description",
				name = "Отображение уведомлений о повышении уровня персонажа и питомца.\n\nАвтор: " .. (self.author or "Неизвестен") .. "\nВерсия: " .. (self.version or "Неизвестна"),
				order = 2,
				width = "full",
			},
		},

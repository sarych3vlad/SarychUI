...
local function IsEnabled()
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.OmniBar then
		return SarychUI.db.profile.addons.OmniBar.enabled ~= false;
	end
...
local DEFAULTS = {
	...
	align                = "CENTER",
	arena                = true,
	battleground         = false,   -- Изменено с true на false
	border               = true,
	center               = true,
	columns              = 8,
	cooldownCount        = true,
	...
	locked               = true,    -- Изменено с false на true
	maxIcons             = 32,
	multiple             = true,
	names                = false,
	padding              = 2,
	...
	unusedAlpha          = 0.45,
	world                = true,
	position             = {        -- Добавлен блок position
		relativeTo        = "UIParent",
		point             = "BOTTOM",
...
function OmniBar:EnsureRuntimeInit()
	if self._suiRuntimeReady then
		return true
	end
	...
	self.spellCasts = self.spellCasts or {}

	if not self._suiEventsRegistered then
		...
		self._suiEventsRegistered = true
	end
...
function OmniBar:OnInitialize()
	...
	-- Регистрируем колбэки для базы данных всегда

...
local function Atr_QueueOrRegisterBlizPanel(f)
	if not f then return end
	if f._suiBlizRegistered then return end
	-- Collect during OnLoad; RegisterInterfaceOptions adds when enabled.
	atrBlizPanels[#atrBlizPanels + 1] = f
	if AuctionatorEnabled and Atr_RegisterInterfaceOptions then
		Atr_RegisterInterfaceOptions()
	end
end

function Atr_RegisterInterfaceOptions()
	if not AuctionatorEnabled then
		return
	end
	if Atr_UIOptionsFrame and not Atr_UIOptionsFrame._suiBlizRegistered then
		Atr_UIOptionsFrame.name = Atr_UIOptionsFrame.name or "Auctionator"
		InterfaceOptions_AddCategory(Atr_UIOptionsFrame)
		Atr_UIOptionsFrame._suiBlizRegistered = true
	end
	for i = 1, #atrBlizPanels do
		local f = atrBlizPanels[i]
		if f and not f._suiBlizRegistered then
			InterfaceOptions_AddCategory(f)
			f._suiBlizRegistered = true
		end
	end
end
...
function Atr_LoadOptionsSubPanel (f, name, title, subtitle)
	...
	Atr_QueueOrRegisterBlizPanel(f);

...
function Atr_GetRecentsList ()
	...
	for x = 1, num do
		if (AUCTIONATOR_SHOPPING_LISTS[x].isRecents) then
...
function Atr_SaveCharacterRecents ()

	local recentsList = Atr_GetRecentsList();

	if (recentsList and recentsList.isRecents) then
		AUCTIONATOR_CHARACTER_RECENTS = zc.CopyDeep (recentsList.items);
	end
end

-----------------------------------------

function Atr_LoadCharacterRecents ()

	if (AUCTIONATOR_CHARACTER_RECENTS == nil) then
		AUCTIONATOR_CHARACTER_RECENTS = {};
	end

	local recentsList = Atr_GetRecentsList();

	if (recentsList and recentsList.isRecents) then
		recentsList.items = zc.CopyDeep (AUCTIONATOR_CHARACTER_RECENTS);
	end
end

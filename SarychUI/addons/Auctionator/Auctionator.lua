AuctionatorVersion = AuctionatorVersion or "???";		-- set from toc upon loading
AuctionatorAuthor  = "Zirco";

-- Global enabled state (SarychUI wrapper sets true on enable)
AuctionatorEnabled = false;

-- Function to check if addon is enabled
local function IsEnabled()
	if AuctionatorEnabled == false then
		return false;
	end
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.Auctionator then
		return SarychUI.db.profile.addons.Auctionator.enabled == true;
	end
	return AuctionatorEnabled == true;
end
...
function Atr_OnLoad()
	...
	-- Set version first, as it's used in other places regardless of enabled state
	if not AuctionatorVersion or AuctionatorVersion == "???" then
		AuctionatorVersion = GetAddOnMetadata("Auctionator", "Version") or "2.6.8";
	end
...
function Atr_AuctionFrameTab_OnClick (self, index, down)
	...
	if not IsEnabled() then
		if auctionator_orig_AuctionFrameTab_OnClick then
			return auctionator_orig_AuctionFrameTab_OnClick(self, index, down);
		end
		return;
	end
	...
	elseif (Atr_IsAuctionatorTab(index)) then
		...
		AuctionFrameTopLeft:SetTexture	("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_topleft");
		AuctionFrameBotLeft:SetTexture	("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_botleft");
		AuctionFrameTop:SetTexture		("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_top");
		AuctionFrameTopRight:SetTexture	("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_topright");
		AuctionFrameBot:SetTexture		("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_bot");
		AuctionFrameBotRight:SetTexture	("Interface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\Atr_botright");
...
function Atr_GetUCIcon (itemName)
	...
	if (scan and scan.absoluteBest and scan.whenScanned ~= 0 and scan.yourBestPrice and scan.yourWorstPrice) then
		...
		if (scan.yourBestPrice <= absBestPrice and scan.yourWorstPrice > absBestPrice) then
			icon = "|TInterface\\AddOns\\SarychUI\\addons\\Auctionator\\Images\\CrossAndCheck:18:18:0:0|t "
			undercutFound = true;

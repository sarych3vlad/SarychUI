...
local function FontTemplate(fs, font, fontSize, fontStyle)
	...
	local general = (E.db and E.db.general) or P.general or {}
	font = font or LSM:Fetch("font", general.font or "Friz Quadrata TT")
	fontSize = fontSize or general.fontSize or 12
	fontStyle = fontStyle or general.fontStyle or "OUTLINE"

	if fontStyle == "OUTLINE" and general.font == "Homespun" and (fontSize > 10 and not fs.fontSize) then
...
local function forceAddapi(object)
	local mt = getmetatable(object).__index
	mt.Size = Size
	mt.Point = Point
...
function E:ApplyToolkitAPI()
	local handled = {["Frame"] = true}
	local object = CreateFrame("Frame")
	forceAddapi(object)
	forceAddapi(object:CreateTexture())
	forceAddapi(object:CreateFontString())
	...
	forceAddapi(GameFontNormal)
end

-- Initial pass: only register if NamePlates (or another embed) has not already done so.
local handled = {["Frame"] = true}
local object = CreateFrame("Frame")
addapi(object)

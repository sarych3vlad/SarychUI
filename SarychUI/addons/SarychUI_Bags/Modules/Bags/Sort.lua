...
function B.Sort(bags, sorter, invertDirection)
	...
	if not next(itemTypes) then BuildSortOrder() end
...
function B:RegisterUpdateDelayed()
	for _, bagFrame in pairs(B.BagFrames) do
		...
		if B.ForceRefreshAllBagSlots then
			B:ForceRefreshAllBagSlots(bagFrame, "register-delayed")
		else
			B:ClearSortingFade(bagFrame)
			B:UpdateAllSlots(bagFrame)
			B:ClearTransientSlotVisuals(bagFrame)
		end
		if B.StopSortSpinner then
			B:StopSortSpinner(bagFrame)
		elseif bagFrame.holderFrame then
...
function B:StopStacking(message, noUpdate)
	...
	if message then
		sortElvDebug("stopped:", message)
	end

	if not noUpdate then
		for _, bagFrame in pairs(B.BagFrames) do
			if bagFrame.registerUpdate then
				if B.ScheduleSortSlotRefresh then
					B:ScheduleSortSlotRefresh(bagFrame)
				elseif B.FinishSortVisualRefresh then
					B:FinishSortVisualRefresh(bagFrame)
				else
					B:ClearSortingFade(bagFrame)
					B:UpdateAllSlots(bagFrame)
					B:ClearTransientSlotVisuals(bagFrame)
		...
		-- Final fallback after API settles.

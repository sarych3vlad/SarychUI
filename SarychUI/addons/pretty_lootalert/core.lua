...
local castbar_offset = config.castbar_offset;

local LOOTALERT_NUM_BUTTONS = config.numbuttons;

-- Смещение по Y при показе кастбара: из SarychUI (вкладка Аддоны) или из config по умолчанию
local function GetCastbarOffsetY()
	local a = SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.pretty_lootalert
	if a then
		if a.castbarOffsetEnabled == false then
			return 0
		end
		local v = a.castbarOffsetY
		if v ~= nil then
			return v
		end
	end
	return castbar_offset
end
...
local function IsEnabled()
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons and SarychUI.db.profile.addons.pretty_lootalert then
		return SarychUI.db.profile.addons.pretty_lootalert.enabled ~= false;
	end
	return pretty_lootalertEnabled;
end

-- /* assets */
local assets = [[Interface\AddOns\SarychUI\addons\pretty_lootalert\assets\]];
		...
		if button and button:IsShown() then
			if button.waitAndAnimOut:GetProgress() <= 0.74 then
				if not previousButton or previousButton == button then
					local adjusted_y = point_y
					if CastingBarFrame and CastingBarFrame:IsShown() then
						adjusted_y = point_y + GetCastbarOffsetY()
					end
					
					if DungeonCompletionAlertFrame1:IsShown() then
						button:SetPoint("BOTTOM", DungeonCompletionAlertFrame1, "TOP", point_x, adjusted_y);
					else
						button:SetPoint("CENTER", DungeonCompletionAlertFrame1, "CENTER", point_x, adjusted_y);
					end
				else
					--button:SetPoint("TOP", previousButton, "BOTTOM", 0, offset_x);
...
function LootAlertFrame_OnEvent(self, event, ...)
	...
	if event == "UNIT_SPELLCAST_START" or event == "UNIT_SPELLCAST_STOP" or event == "UNIT_SPELLCAST_INTERRUPTED" or event == "UNIT_SPELLCAST_FAILED" then
		local unit = ...;
		if unit == "player" then

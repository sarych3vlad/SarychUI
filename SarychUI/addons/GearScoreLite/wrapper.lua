-- GearScoreLite Wrapper for SarychUI
-- Manages loading and enabling/disabling of GearScoreLite addon

local ADDON_NAME = "GearScoreLite"
local addonPath = "Interface\\AddOns\\SarychUI\\addons\\GearScoreLite\\"

-- Create wrapper module
local wrapper = {
	name = ADDON_NAME,
	author = "Mirrikat45, Gnomezilla [Warmane-Icecrown(A)]",
	version = "3x05",
	enabled = false,
	loaded = false,
}
...
local function GetRuntimeEnabledFromDB()
	...
	return GearScoreLiteEnabled ~= false
end
...
function wrapper:Initialize()
	...
	-- Ensure original addon saved variables are initialized even without GearScoreLite's TOC/ADDON_LOADED
	if not GS_Settings and _G.GS_DefaultSettings then
		GS_Settings = GS_DefaultSettings
	end
	if not GS_Data then
		GS_Data = {}
	end
	if GetRealmName and not GS_Data[GetRealmName()] then
	...
	if SarychUI and SarychUI.db and SarychUI.db.profile and SarychUI.db.profile.addons then
		if SarychUI.db.profile.addons[ADDON_NAME] and SarychUI.db.profile.addons[ADDON_NAME].enabled then
			...
			GearScoreLiteEnabled = true
			self.enabled = true
			self.loaded = true
		else
			-- Set global enabled state
			GearScoreLiteEnabled = false
			self.enabled = false
		end
	else
...
function wrapper:Disable()
	...
	-- Hide personal gear score when disabled
	...
	return {
		...
		args = {
			enabled = {
				type = "toggle",
				name = "Включить",
				desc = "Включить/выключить GearScoreLite",
				order = 1,
				get = function()
			...
			description = {
				type = "description",
				name = "Помогает быстро и легко оценить уровень экипировки игрока (GearScore).\n\nАвтор: " .. (self.author or "Неизвестен") .. "\nВерсия: " .. (self.version or "Неизвестна"),
				order = 2,
				width = "full",
			},
			commands = {
				type = "description",
				name = "|cFFFFD700Команды:|r\n/gs player - Переключить отображение очков на игроках\n/gs item - Переключить отображение очков для предметов\n/gs level - Переключить информацию об уровне предметов\n/gs reset - Сбросить настройки GearScore по умолчанию\n/gs compare - Переключить отображение сравнительной информации между вами и целью",
				order = 3,
				width = "full",
			},
		},

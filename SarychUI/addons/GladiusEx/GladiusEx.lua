...
function GladiusEx:OnInitialize()
    ...
    -- libsharedmedia
    local mediaPath = [[Interface\AddOns\SarychUI\addons\GladiusEx\media]]
    LSM:Register("statusbar", "Minimalist (GladiusEx)", mediaPath .. [[\Minimalist]])
    LSM:Register("statusbar", "Wglass (GladiusEx)", mediaPath .. [[\Wglass]])
    LSM:Register("font", "Designosaur (GladiusEx)", mediaPath .. [[\Designosaur-Regular.ttf]])
    LSM:Register("font", "Designosaur Italic (GladiusEx)", mediaPath .. [[\Designosaur-Italic.ttf]])
...
function GladiusEx:OnDisable()
    ...
    -- AceDB uses CallbackHandler: UnregisterAllCallbacks (not UnregisterAllEvents).
    if self.dbi then
        if self.dbi.UnregisterAllCallbacks then
            self.dbi.UnregisterAllCallbacks(self)
        elseif self.dbi.UnregisterAllEvents then
            self.dbi.UnregisterAllEvents(self)
        end
    end
    if self.HideFrames then
        self:HideFrames()
    end
end

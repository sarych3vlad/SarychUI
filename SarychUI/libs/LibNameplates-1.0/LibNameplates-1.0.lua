...
do
	if not lib.scanForPlate then
		...
		-- Reused buffer: GetChildren() must be called once per scan, not once per
		-- index, or a burst of new plates costs O(children^2).
		local childBuffer = {}
		...
		lib.scanForPlate:SetScript("OnUpdate", function()
			...
			if curChildren ~= prevChildren then
				if curChildren > prevChildren then
					FillFrom(childBuffer, WorldFrame:GetChildren())

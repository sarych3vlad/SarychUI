...
function Core.Enable()
	...
	Core.RegisterEvent("PLAYER_FOCUS_CHANGED")
	Core.RegisterEvent("ARENA_OPPONENT_UPDATE")
	Core.RegisterEvent("UNIT_PET")
	Core.RefreshWatchedGUIDs()
...
function Core.CreateAreaTrigger(timestamp, srcGUID, srcName, triggerGUID, triggerName, spellid, spellschool)
	...
	local effectInfo = Effects[spellid]
	if not effectInfo then return end
...
-- The affiliation filter keeps the combat log cheap, but it also drops units that
-- consumers still draw absorb bars for: target, focus and arena enemies never carry
-- a group flag. Those few GUIDs are kept here so their shields stay visible without
-- opening the filter up to every unit in the world.
local watchedUnits = {
	"target", "focus",
	"arena1", "arena2", "arena3", "arena4", "arena5",
	"arenapet1", "arenapet2", "arenapet3", "arenapet4", "arenapet5"
}
local arenaPetOwners = {arena1 = true, arena2 = true, arena3 = true, arena4 = true, arena5 = true}
local watchedGUIDs = {}

local function RefreshWatchedGUIDs()
	for guid in pairs(watchedGUIDs) do
		watchedGUIDs[guid] = nil
	end
	for i = 1, #watchedUnits do
		local guid = UnitGUID(watchedUnits[i])
		if guid then
			watchedGUIDs[guid] = true
		end
	end
end

Core.WatchedGUIDs = watchedGUIDs
Core.RefreshWatchedGUIDs = RefreshWatchedGUIDs
...
Events.ARENA_OPPONENT_UPDATE = Events.PLAYER_TARGET_CHANGED

function Events.UNIT_PET(unit)
	if arenaPetOwners[unit] then
		RefreshWatchedGUIDs()
	end
end
...
function Events.COMBAT_LOG_EVENT_UNFILTERED(timestamp, etype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
	if lib.CheckFlags and not CheckFlags(srcFlags, dstFlags)
		and not watchedGUIDs[dstGUID] and not watchedGUIDs[srcGUID] then return end
